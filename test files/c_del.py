            ...
            for update in response.update.update:
                if(update.val):
                    if target_info["target"] == ("leaf1", GNMI_PORT) or target_info["target"] == ("leaf2", GNMI_PORT):
                        if int(update.val.json_ietf_val) == 1:
                        ...
                        if int(update.val.json_ietf_val) == 2:
                            # Only record subsequent changes
                            timestamp = datetime.now()
                            changes_timestamps.append(timestamp)
                            print(f"Change detected on {target_info['target']} at {timestamp}")
...
if __name__ == "__main__":
    with open('TimetoUpdateDelete.txt', 'a') as file:
        for r in replicas:
            n = 0
            while n < 20:
                result_container = {}
                ...
                print(f'Time to pods leave running: {time_to_running - initial_time}, Time to network converge: {time_to_converge - initial_time}. Iteration {n}:{r}')
                time.sleep(0.5)
                reset_pods(r)
