...
spoofed_ip = ["192.168.2.15", "192.168.2.16", "192.168.2.17", "192.168.2.18", "192.168.2.19", "192.168.2.20", "192.168.2.21", 
              "192.168.2.22", "192.168.2.23", "192.168.2.24", "192.168.2.25", "192.168.2.26", "192.168.2.27", "192.168.2.28", "192.168.2.29"]
        ...
        while self.connected:
            ...
            if p.haslayer(TCP) and p.haslayer(Raw) and p[TCP].dport == self.sport and p[IP].dst == self.srcIp:
                    ...
                    if p[0].haslayer(Raw):
                        ...
                        #print(f"Response from {destination_ip}:\n{http_response.decode('utf-8', errors='replace')}")
                        match = re.search(r'Server address: (\d+\.\d+\.\d+\.\d+):\d+', http_response.decode('utf-8'))
                        if match:
                            #print("IP QUE ENVIOU: " + match.group(1))
